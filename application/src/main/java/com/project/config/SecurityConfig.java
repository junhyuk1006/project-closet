    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, CustomOAuth2UserService customOAuth2UserService) throws Exception {
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/", "/error", "/**", "/MyPoint","/login", "/oauth2/authorization/**", "/api/auth/signup", "/api/auth/signin").permitAll() // 메인 홈 및 로그인 경로 접근 허용
                .requestMatchers("/api/itemDetail/**","/api/posts", "/api/basket/**", "/api/orders/**", "/api/payment/**").permitAll() // 특정 경로 인증 없이 접근 허용
