    ...
    public DefaultSecurityFilterChain securityFilterChain(HttpSecurity http, CustomOAuth2UserService customOAuth2UserService) throws Exception {
        ...
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/", "/**", "/MyPoint","/login", "/oauth2/authorization/**", "/api/auth/**", "/api/email/**").permitAll() // 메인 홈 및 로그인 경로 접근 허용
                .requestMatchers("/api/itemDetail/**","/api/board", "/api/basket/**", "/api/coordi", "/api/replies").permitAll() // 특정 경로 인증 없이 접근 허용
