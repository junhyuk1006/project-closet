...
function ReviewInput({ activeTab, userId, productId }) {
    ...
    const fetchReviews = async () => {
        try {
            const response = await fetch(`http://localhost:80/api/findAllReview/${productId}`);
    ...
    const handleUpdate = async (reviewId, updatedContent) => {
        try {
            const updatedData = { review_content: updatedContent };
            const response = await fetch(`http://localhost:80/api/updateReview/${reviewId}`, {
                method: "PUT",
    ...
    const handleDeactivate = async (reviewId) => {
        try {
            const response = await fetch(`http://localhost:80/api/deactivateReview/${reviewId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
            });
    ...
    const handleActivate = async (reviewId) => {
        try {
            const response = await fetch(`http://localhost:80/api/activateReview/${reviewId}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
            });
                    ...
                    <div className="col-sm-10 col-md-8 col-lg-6 m-lr-auto">
                        <div className="p-b-30 m-lr-15-sm">
                            ...
                            {currentReviews.map((review) => (
                                <div key={review.reviewId} className="flex-w-review flex-t p-bst-68">
                                    <div className="wrap-pic-s size-109 bor0 of-hidden m-r-18 m-t-6">
                                        <img src={`images/${review.profileImage}`} alt={review.nickname}/>
                                    </div>
                                    <div className="size-207">
                                        <div className="flex-w-review flex-sb-m p-b-17-review">
                                            <span className="mtext-107 cl2 p-r-20">{review.nickname}</span>
                                                        ...
                                                        {dropdownStates[review.reviewId] && (
                                                            <div className="dropdown-menu" onClick={handleDropdownClick}>
                                                                {review.status === "inactive" ? (
                                                                    <button
                                                                        onClick={() => handleActivate(review.reviewId)}
                                                                    ...
                                                                    <>
                                                                        <button
                                                                            onClick={() => {
                                                                                ...
                                                                                if (updatedContent) handleUpdate(review.reviewId, updatedContent);
                                                                            }}
                                                                            className="dropdown-item"
                                                                        >
                                                                            수정
                                                                        ...
                                                                        <button
                                                                            onClick={() => handleDeactivate(review.reviewId)}
                                                                            className="dropdown-item"
                                                                        >
                                                                            비활성화
                                                                        </button>
